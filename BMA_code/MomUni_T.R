...
foreach (scen_ii = 32:33) %dopar%{
  foreach (ii = 1:5) %dopar%{
    ...
    tmp = read.csv("Input/X0TU.csv",header = FALSE)
    ...
    # Save output
    mu0str = paste(sprintf("Output/muUT_%d_scen%d",yearY,scen_ii),"job", JOBID, currentDate, ".csv", sep="_")
    nu0str = paste(sprintf("Output/nuUT_%d_scen%d",yearY,scen_ii),"job", JOBID, currentDate, ".csv", sep="_")
    lambdastr = paste(sprintf("Output/lambdaUT_%d_scen%d",yearY,scen_ii),"job", JOBID, currentDate, ".csv", sep="_")
